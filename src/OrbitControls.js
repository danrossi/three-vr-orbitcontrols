...
/**
 * @type {TouchEvent[]} - touch pointer events
 */
let _pointers = [];

/**
 * Add the touch pointer
...
/**
 * Get the touch pointer position
 * @param {TouchEvent} event - the touch event
 */
...
export default class OrbitControls extends EventDispatcher {
  ...
  resetXAxis() {
    this.sphericalDelta.theta = this.sphericalDelta.theta < 0 ? 0.5 : -0.5;
  ...
  handleMouseMoveRotate(event) {
    ...
    const element =
      this.domElement === document ? this.domElement.body : event.target;
  ...
  onPointerDown(event) {
    ...
    if (
      this.domElement !== event.target &&
      this.domElement !== event.target.parentNode
    )
      return;
  ...
  connect() {
    ...
    this.domElement.addEventListener('wheel', this.onMouseWheelRef, {
      passive: false,
    });
  }
